...
- Create a new `QueryClient` instance **inside of your app, and on an instance ref (or with memoization). This ensures that data is not shared between different users and requests, while still only creating the QueryClient once per component lifecycle.**
